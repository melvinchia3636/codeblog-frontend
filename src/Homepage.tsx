...
const Landing: React.FC<ILanding> = ({page, setPage}: ILanding): JSX.Element => {
    ...
    return <>
        ...
        {Array(5).fill(0).map((_, i) => <div key={i} className={`layer layer-${i+1} wh-full left-0 -top-full absolute z-10 ${[4, 5, 7, 8, 9, 10, 12, 13, 14].includes(page) ? "active" : ""}`}></div>)}
        <div className={`${anim} animate__delay-1s animate__fadeIn wh-full left-0 top-0 bg-black-dark absolute ${[4, 5, 6].includes(page) ? "" : "hidden"} ${page === 6 ? "section-leave" : ""}`}>
            ...
            <p className="${anim} absolute z-0 bottom-0 right-8 text-enormous leading-none font-bold font-Chakra tracking-wider text-black-light animate__fadeInUp animate__delay-2s text-dark-inner">01A</p>
        </div>
