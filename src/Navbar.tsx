    ...
    return <nav>
        ...
        <Anime initial={[
            ...
            <div className="text-white z-10 font-Chakra flex gap-16 sidebar transform origin-top-left -rotate-90 absolute bottom-2 left-6">
                {sideNavItem.map((e, i) => <button key={`sideNav-${i}`} className={`uppercase tracking-widerr text-lg transition-color font-bold ${[1, 2, 3, 6, 11].includes(page) ? (!i ? "text-black-dark" : "text-yellow-900") : (!i ? "text-yellow-500" : "")}`}>{e}</button>)}
            </div>
        </Anime>
        <Anime initial={[
            {
